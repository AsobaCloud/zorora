    ...
    try:
        ...
        for prefix in response.get('CommonPrefixes', []):
            ...
            if len(folder) == 10 and folder[4] == '-' and folder[7] == '-':
                if start_date.strftime('%Y-%m-%d') <= folder <= end_date.strftime('%Y-%m-%d'):
...
def _fetch_articles_from_date(s3_client, date_folder: str, max_results: int = 500) -> List[Dict]:
...
def _parse_date(date_str: str) -> str:
    ...
    for fmt in formats:
        try:
            return datetime.strptime(date_str[:len(fmt.replace('%', 'YY'))], fmt).strftime('%Y-%m-%d')
...
def fetch_newsroom_s3_raw(days_back: int = 90, max_results: int = 3000) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for date_folder in date_folders:
            ...
            articles = _fetch_articles_from_date(s3_client, date_folder, max_results - len(all_articles))
...
def fetch_newsroom_cached(max_results: int = 100):
    ...
    if stale_articles:
        logger.warning(f"S3 fetch failed, using stale cache: {len(stale_articles)} articles")
        return (stale_articles[:max_results], "Using cached data — newsroom data refresh unavailable")
